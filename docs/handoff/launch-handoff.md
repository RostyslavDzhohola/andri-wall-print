...
- Push Convex functions after the schema changes: `globalGenerationCap`, `funnelEvents`, concept AR fields on `aiConceptDrafts`, and `galleryEntries`. Completed legacy table removals are target-state checks only; do not redeploy deleted tables or rerun old removal migrations. Use the Convex dev/sync/deploy flow only after confirming the target deployment.
...
- To remove a community item, follow `production-operations.md`: hide it first, then note that direct storage URLs remain reachable unless a verified asset-revocation request also authorizes deletion or access-controlled replacement of the exact poster/GLB/USDZ objects.
