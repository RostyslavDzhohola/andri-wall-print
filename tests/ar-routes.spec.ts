...
test("homepage renders the native AR sample", async ({ page }) => {
  ...
  await expect(page.locator("model-viewer")).toHaveAttribute("src", "/ar/dragon-wall-print.glb");
  await expect(page.locator("model-viewer")).toHaveAttribute("ios-src", "/ar/dragon-wall-print.usdz");
